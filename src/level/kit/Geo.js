...
export class ProxySet {
  ...
  /**
   * Seal the dead cracks between building shells.
   *
   * THE PROBLEM, MEASURED. A player reported "narrow corridors between buildings
   * that feel like a flaw", and tools/level-probe.mjs found 147 m2 of walkable
   * space at or under 1.5 m wide, in 79 clusters, the narrowest 0.52 m and the
   * worst an 8.3 m long half-metre crack. They are not designed alleys — the
   * alley in this level is 7 m wide — they are the seams left where two building
   * shells stand near each other, and a player who walks into one is stuck in a
   * gap that leads nowhere.
   *
   * WHY THIS IS NOT A BLANKET "CLOSE THE NARROW GAPS" PASS. A doorway is also
   * about a metre wide. So is the mouth of a stairwell, and the gap between a
   * building and its own doorframe. Morphologically closing every narrow gap
   * would wall the player out of every interior in the level, which is a far
   * worse bug than the one being fixed — and it would not be obvious, because
   * the level would still look right.
   *
   * The property that separates them is CONNECTIVITY, not width. A doorway joins
   * two open areas; a crack is a cul-de-sac. So a narrow region is sealed only
   * when it touches at most one wide open region. Doorways, alley mouths and
   * stairwell entrances all touch two and are left alone by construction rather
   * than by a list of exceptions.
   *
   * Only the chest band is considered solid (0.7 .. 1.7 m). Anything that exists
   * purely below it is cover to be walked over or mantled, not a wall, and
   * treating a knee-high crate as solid would invent narrow gaps beside every
   * piece of low cover.
   *
   * @param opts.maxWidth   a gap this wide or narrower is a candidate (metres)
   * @param opts.maxArea    refuse to seal a cul-de-sac larger than this, so a
   *                        genuine room can never be walled off silently
   * @returns a report — what was sealed, and what was spared and why
   */
  sealDeadCracks(opts = {}) {
    const CELL = opts.cell ?? 0.26;
    const MAX_W = opts.maxWidth ?? 1.2;
    const MAX_AREA = opts.maxArea ?? 14;
    const LO = opts.bandLo ?? 0.7, HI = opts.bandHi ?? 1.7;
    const P = this.pos;

    /* --- 1. footprint of everything solid at chest height ----------------- */
    // Bounds come only from band-overlapping geometry, so the 280 m ground slab
    // (which sits entirely below the band) cannot inflate the grid.
    let minX = Infinity, maxX = -Infinity, minZ = Infinity, maxZ = -Infinity;
    const tris = [];
    for (let i = 0; i < P.length; i += 9) {
      const y0 = Math.min(P[i + 1], P[i + 4], P[i + 7]);
      const y1 = Math.max(P[i + 1], P[i + 4], P[i + 7]);
      ...
      tris.push(ax, az, bx, bz);
      if (ax < minX) minX = ax; if (bx > maxX) maxX = bx;
      if (az < minZ) minZ = az; if (bz > maxZ) maxZ = bz;
    }
    if (!tris.length) return { sealed: 0, sealedAreaM2: 0, spared: [], note: 'no chest-band geometry' };

    const pad = 2;
    minX -= pad; minZ -= pad; maxX += pad; maxZ += pad;
    const gw = Math.ceil((maxX - minX) / CELL) + 1;
    const gd = Math.ceil((maxZ - minZ) / CELL) + 1;
    const solid = new Uint8Array(gw * gd);
    // Each proxy is a box or a ramp, so a triangle's 2D bounding box is either
    // its own footprint or a subset of its parent box's — marking the bbox is
    // therefore exact for these shapes, and it is also the only thing that
    // catches a vertical wall quad, whose projected AREA is zero. (The minimap
    // hit the same wall: a filled projection of a vertical face draws nothing.)
    for (let t = 0; t < tris.length; t += 4) {
      const x0 = Math.max(0, Math.floor((tris[t] - minX) / CELL));
      const z0 = Math.max(0, Math.floor((tris[t + 1] - minZ) / CELL));
      const x1 = Math.min(gw - 1, Math.ceil((tris[t + 2] - minX) / CELL));
      const z1 = Math.min(gd - 1, Math.ceil((tris[t + 3] - minZ) / CELL));
      for (let z = z0; z <= z1; z++) {
        const row = z * gw;
    ...
    /* --- 2. corridor width of every free cell ----------------------------- */
    const narrowLimit = Math.max(1, Math.round(MAX_W / CELL));
    const narrow = new Uint8Array(gw * gd);
    for (let z = 0; z < gd; z++) {
      for (let x = 0; x < gw; x++) {
        const c = z * gw + x;
        if (solid[c]) continue;
        let sx = 1, sz = 1;
        for (let i = x - 1; i >= 0 && !solid[z * gw + i]; i--) sx++;
        for (let i = x + 1; i < gw && !solid[z * gw + i]; i++) sx++;
        for (let i = z - 1; i >= 0 && !solid[i * gw + x]; i--) sz++;
        for (let i = z + 1; i < gd && !solid[i * gw + x]; i++) sz++;
        if (Math.min(sx, sz) <= narrowLimit) narrow[c] = 1;
      }
    }

    /* --- 3. label the WIDE free regions ----------------------------------- */
    // These are the rooms and streets. A narrow region touching two of them is
    // a connector and must survive.
    const label = new Int32Array(gw * gd).fill(-1);
    let regions = 0;
    const stack = [];
    for (let s = 0; s < gw * gd; s++) {
      if (solid[s] || narrow[s] || label[s] >= 0) continue;
      const id = regions++;
      stack.push(s);
      label[s] = id;
      while (stack.length) {
        ...
        if (x > 0) { const n = c - 1; if (!solid[n] && !narrow[n] && label[n] < 0) { label[n] = id; stack.push(n); } }
        if (x < gw - 1) { const n = c + 1; if (!solid[n] && !narrow[n] && label[n] < 0) { label[n] = id; stack.push(n); } }
        if (z > 0) { const n = c - gw; if (!solid[n] && !narrow[n] && label[n] < 0) { label[n] = id; stack.push(n); } }
        if (z < gd - 1) { const n = c + gw; if (!solid[n] && !narrow[n] && label[n] < 0) { label[n] = id; stack.push(n); } }
      }
    }

    /* --- 4. cluster the narrow cells and judge each cluster ---------------- */
    ...
    for (let s = 0; s < gw * gd; s++) {
      ...
      const at = (() => {
        const x = members[0] % gw, z = (members[0] - x) / gw;
        return [+(minX + x * CELL).toFixed(1), +(minZ + z * CELL).toFixed(1)];
      })();
      if (touches.size >= 2) { spared.push({ at, areaM2: +areaM2.toFixed(1), reason: `connects ${touches.size} open areas` }); continue; }
      if (areaM2 > MAX_AREA) { spared.push({ at, areaM2: +areaM2.toFixed(1), reason: `larger than ${MAX_AREA} m2 — treated as a real space` }); continue; }
      toSeal.push(members);
      sealedCells += members.length;
    }

    /* --- 5. emit the fill, merged into runs along X ------------------------ */
    // One box per contiguous row rather than per cell: the same volume for a
    // fraction of the triangles, which matters because this geometry joins the
    // bullet and capsule BVH.
    const fill = new Uint8Array(gw * gd);
    for (const m of toSeal) for (const c of m) fill[c] = 1;
    let boxes = 0;
    const H = HI - LO + 1.6;                      // seal well above head height
    for (let z = 0; z < gd; z++) {
      let x = 0;
      while (x < gw) {
        ...
        const w = (e - x + 1) * CELL;
        this.box(minX + (x + (e - x + 1) / 2) * CELL, LO + H / 2 - 0.7,
          minZ + (z + 0.5) * CELL, w, H, CELL);
        boxes++;
        x = e + 1;
      }
    }
    ...
    return {
      ...
      boxes,
      wideRegions: regions,
      spared: spared.sort((a, b) => b.areaM2 - a.areaM2).slice(0, 12),
